...
function OauthManager(settings, oauth) {
  ...
  function getDatabases() {
    return getToken().then(oauth.searchRequestFunction).then(oauth.searchRequestHandler);
  }
  ...
  function getChosenDatabaseFile(dbInfo, attempt) {
    return getToken().then(function (accessToken) {
      return oauth
        .fileRequestFunction(dbInfo, accessToken)
  ...
  function ensureOriginPermissions() {
    return chromePromise.permissions
      .contains({
